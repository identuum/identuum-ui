...
describe("unconsumed TOTP windows", () => {
  it("never issues the same step twice: current, then next, then previous, then nothing", () => {
    ...
    const a = claimUnconsumedTOTPWindow(SECRET, T0);
    const b = claimUnconsumedTOTPWindow(SECRET, T0);
    const c = claimUnconsumedTOTPWindow(SECRET, T0);
    const d = claimUnconsumedTOTPWindow(SECRET, T0);
    ...
    expect(presentedTOTPSteps(SECRET, T0)).toEqual([step - 1, step, step + 1]);
  });
  ...
  it("waits for the next step only when all three windows are spent, and then issues a step nobody presented", async () => {
    ...
    const step = totpStep(T0);
    const first = await unconsumedTOTP(SECRET, f.clock);
    const second = await unconsumedTOTP(SECRET, f.clock);
    const third = await unconsumedTOTP(SECRET, f.clock);
    expect(f.sleeps).toEqual([]);
    expect(new Set([first, second, third]).size).toBe(3);
    const fourth = await unconsumedTOTP(SECRET, f.clock);
    ...
    expect(presentedTOTPSteps(SECRET, f.now)).toEqual([step - 1, step, step + 1, step + 2]);
  });
  ...
  it("a fresh-step retry always comes from a step that was not current or next when the refused code was issued", async () => {
    ...
    const f = fakeClock(T0);
    const step = totpStep(T0);
    const refused = await unconsumedTOTP(SECRET, f.clock); // step
    expect(refused).toBe(totpCodeForStep(SECRET, step));
    const retry = await unconsumedTOTPAfterFreshStep(SECRET, f.clock);
    expect(f.sleeps.length).toBe(1);
    expect(totpStep(f.now)).toBe(step + 1);
  ...
  it("two secrets do not share a ledger", () => {
    resetTOTPLedgerForTests(null);
    const other = "GEZDGNBVGY3TQOJQ";
    const a = claimUnconsumedTOTPWindow(SECRET, T0);
    const b = claimUnconsumedTOTPWindow(other, T0);
  ...
  it("FAILS LOUDLY when even a fresh step yields nothing, naming the presented steps, and deletes nothing", async () => {
    ...
    for (let i = 0; i < 3; i++) claimUnconsumedTOTPWindow(SECRET, f.now);
    const stuck = { now: () => f.now, sleep: async () => {} };
    await expect(unconsumedTOTP(SECRET, stuck)).rejects.toBeInstanceOf(NoUnconsumedTOTPWindowError);
    await expect(unconsumedTOTP(SECRET, stuck)).rejects.toThrow(
    ...
    expect(presentedTOTPSteps(SECRET, f.now)).toEqual([step - 1, step, step + 1]);
  });

  it("the refusal-after-fresh-step sentence names the case and promises nothing was deleted", () => {
    resetTOTPLedgerForTests(null);
    claimUnconsumedTOTPWindow(SECRET, T0);
    const msg = refusedAfterFreshStepMessage("site_admin mfa login", SECRET, T0);
    expect(msg).toContain("a step this run had never presented");
    expect(msg).toContain(`presented steps for this secret: ${totpStep(T0)}`);
