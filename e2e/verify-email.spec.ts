...
test.describe("/verify-email page — safe states", () => {
  ...
  test("renders a safe error state for an unrecognised token [VERIFY-EMAIL-SAFE-1]", async ({
    page,
  }) => {
  ...
  test("resend form shows generic sent message after submission [VERIFY-RESEND-GENERIC-1]", async ({
    page,
  }) => {
    await page.goto("/verify-email?token=invalid_e2e_test_token_verify");
