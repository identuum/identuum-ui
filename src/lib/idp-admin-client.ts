 ...
 *
 * activation semantics (MEASURED against idp-oss HandleCreateOrganization,
 * 2026-08-31 — the previous note here described an "air-gapped mode" and an
 * `IsAirGapped` backend flag that DO NOT EXIST in OSS; air-gapped is a CE
 * feature, and OSS supports both email-configured and email-not-configured
 * operation):
 *   - When admin_email is supplied, OSS ALWAYS returns activation_token —
 *     not only when email delivery is unconfigured.
 *   - The token is a raw one-time credential, not a URL. Expiry: 24h.
 *   - activation_url accompanies it (THE-UNUSABLE-TOKEN) when the IdP knows
 *     the UI's public base URL. When it does not, the server sends
 *     activation_url_unavailable explaining why, naming the setting —
 *     EXACTLY ONE of the two is ever present, and neither is guessed here.
 */
export type CreateOrgResult =
  | {
      ...
      name: string;
      domain: string;
      /** Raw one-time token; not a URL. Always present when admin_email was sent. */
      activationToken: string | null;
      /** Ready-to-open activation link, when the server could build one. */
      activationUrl: string | null;
      /** Why no link exists (names the setting). Never set together with activationUrl. */
...
export async function createOrganization(opts: CreateOrgOptions): Promise<CreateOrgResult> {
  ...
  try {
    ...
    // Sanitize to the UI-safe shape. The server sends the raw token plus
    // EXACTLY ONE of activation_url / activation_url_unavailable
    // (THE-UNUSABLE-TOKEN). Nothing is synthesized here: if the server could
    // not build a link, the UI shows the server's reason, never a guess.
    const rawToken = typeof data.activation_token === "string" ? data.activation_token.trim() : "";
    const rawUrl = typeof data.activation_url === "string" ? data.activation_url.trim() : "";
    const rawUnavailable =
      typeof data.activation_url_unavailable === "string"
        ? data.activation_url_unavailable.trim()
        : "";
    return {
      ok: true,
      id: String(org.id ?? ""),
      ...
      activationUrl: rawUrl || null,
      activationUrlUnavailable: rawUnavailable || null,
    };
  } catch {
 ...
 * user ID. R2 (ruled 2026-08-21): after a successful change the IdP revokes
 * every OTHER session and all refresh tokens; the changing session stays valid.
 *
 * Security:
...
// signal — no recovery URL or email is generated by this primitive, so it
// works with email delivery unconfigured. (An operator can run this reset
...
//   - Response (MEASURED against idp-oss HandleApproveUser, 2026-08-31):
//     200 with the BARE safe-user object — {id, organization_id, email,
//     role, ...}. There is no `success` envelope, no `message`, and no
//     `activation_url`: OSS approve only clears the pending flag, it issues
//     no activation token.
//
//     THE-UNUSABLE-TOKEN fixed a real defect here: this client gated success
//     on `data.success`, a field the server never sends, so a SUCCESSFUL
//     approve fell through to the error branch and told the operator it had
//     failed. The gate is now the shape the server actually returns.
//
//     activationUrl stays in the result type because identuum-ui also serves
//     CE, whose approve MAY return one; on OSS it is always null. It is
...
export async function approveUserRegistration(userId: string): Promise<ApproveRegistrationResult> {
  ...
  try {
    ...
    // Success is the SHAPE the server returns (a safe-user object carries an
    // id), not a `success` flag it never sends.
